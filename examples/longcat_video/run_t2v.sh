...
if [ "$RANK_ARG" -eq 0 ]; then
    echo "Running as Master Node (Rank 0, 2 GPUs - VAE)"
    export NODE_RANK=0
    export CUDA_VISIBLE_DEVICES=2,3
elif [ "$RANK_ARG" -eq 1 ]; then
    echo "Running as Worker Node (Rank 1, 8 GPUs - Training)"
    export NODE_RANK=1
...
# Single-node config 
EXPR_NAME=longcat_t2v_debug
N_VAE=$N_GPU_FOR_DATA
TRAIN_SCRIPT=${2:-"examples/longcat_video/pretrain_t2v.py"}
CONFIG_PATH=${3:-"examples.longcat_video.config.longcat_t2v.config"}
if [ $# -ge 3 ]; then
    shift 3
elif [ $# -ge 2 ]; then
    shift 2
...
if [[ "$TRAIN_SCRIPT" == *"pretrain_i2v.py"* ]] && [[ "$CONFIG_PATH" != *"longcat_i2v"* ]]; then
    echo "Invalid config/script pair: TRAIN_SCRIPT=$TRAIN_SCRIPT but CONFIG_PATH=$CONFIG_PATH"
    exit 1
fi
if [[ "$TRAIN_SCRIPT" == *"pretrain_t2v.py"* ]] && [[ "$CONFIG_PATH" != *"longcat_t2v"* ]]; then
    echo "Invalid config/script pair: TRAIN_SCRIPT=$TRAIN_SCRIPT but CONFIG_PATH=$CONFIG_PATH"
    exit 1
fi
...
echo "Using MASTER_ADDR ($MASTER_ADDR). Ensure this is the IP of the Master Node (Rank 0)."
