...
def get_cp_size():
    global cp_size
    if cp_size == 1 and mpu.model_parallel_is_initialized():
...
def get_cp_group():
    global cp_group
    if cp_group is None and mpu.model_parallel_is_initialized():
...
def get_cp_rank():
    global cp_rank
    if cp_rank == 0 and mpu.model_parallel_is_initialized():
...
def get_cp_rank_list():
    ...
    if cp_ranks == None:
        ...
        if group is not None:
            cp_ranks = torch.distributed.get_process_group_ranks(group)
        else:
            # Fallback if no group is found, though this shouldn't happen if initialized
            cp_ranks = list(range(dist.get_world_size()))
    return cp_ranks
...
def cp_broadcast(tensor, cp_index=0):
    ...
    torch.distributed.broadcast(tensor, cp_ranks[cp_index], group=group)
...
def split_tensor_in_cp_2d(input, dim_hw, split_hw):
    ...
    group = get_cp_group()
    cp_size_dynamic = dist.get_world_size(group=group) if group is not None else dist.get_world_size()
    assert cp_size_dynamic == split_h * split_w
    ...
    cp_rank = get_cp_rank() if group is not None else dist.get_rank()
