...
class ParallelLongCatModel(ContextParallelMixin, TransformerGeneralMixin, LongCatVideoTransformer3DModel):
    def __init__(self, config):
        ...
        # Align num_heads to CP world size for ulysses all-to-all on head dim
        if mpu.model_parallel_is_initialized():
            cp_size = mpu.get_context_parallel_world_size()
        else:
            cp_size = (cp_split_hw[0] * cp_split_hw[1]) if cp_split_hw is not None else 1
        try:
            from teletron.utils import get_args
            args = get_args()
            print(f"[ParallelLongCatModel] config_path={args.config_path}", flush=True)
        except Exception as _e:
            print(f"[ParallelLongCatModel] config_path=<unavailable> ({_e})", flush=True)
        print(f"[ParallelLongCatModel] cp_split_hw={cp_split_hw}, num_heads={num_heads}, dim={dim}, cp_size={cp_size}", flush=True)
        if cp_size > 1:
            if num_heads % cp_size != 0:
                num_heads = ((num_heads // cp_size) + 1) * cp_size
            
            # Ensure dim is divisible by num_heads (for split) AND head_dim is divisible by 8 (for RoPE)
            target_divisor = num_heads * 8
            if dim % target_divisor != 0:
                dim = ((dim // target_divisor) + 1) * target_divisor
                print(f"[ParallelLongCatModel] Adjusted dim to {dim} to satisfy CP (divisible by num_heads={num_heads}) and RoPE (head_dim divisible by 8)", flush=True)
