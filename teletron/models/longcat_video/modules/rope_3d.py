...
def broadcat(tensors, dim=-1):
    ...
    if len(shape_lens) != 1:
        print(f"DEBUG: broadcat shapes: {[t.shape for t in tensors]}")
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def precompute_freqs_cis_3d(self, grid_size):
        ...
        # Ensure grids are 1D
        if grid_t.ndim == 0: grid_t = grid_t.unsqueeze(0)
        if grid_h.ndim == 0: grid_h = grid_h.unsqueeze(0)
        if grid_w.ndim == 0: grid_w = grid_w.unsqueeze(0)
