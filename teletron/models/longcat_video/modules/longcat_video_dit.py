...
class LongCatVideoTransformer3DModel(
    ...
    def forward(
        ...
        do_cp = self.cp_split_hw is not None and (self.cp_split_hw[0] * self.cp_split_hw[1] > 1) #向前传播时，判断是否需要进行上下文并行
        if do_cp and (hidden_states.shape[1] == N_t * N_h * N_w): 
            hidden_states = rearrange(hidden_states, "B (T H W) C -> B T H W C", T=N_t, H=N_h, W=N_w)
            hidden_states = context_parallel_util.split_cp_2d(hidden_states, seq_dim_hw=(2, 3), split_hw=self.cp_split_hw) #调用底层的切分工具
        ...
        if do_cp:
            split_h, split_w = self.cp_split_hw
            split_tokens = N_t * (N_h // split_h) * (N_w // split_w)
            if hidden_states.shape[1] == split_tokens:
