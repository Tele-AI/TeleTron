...
from teletron.models.flow_match import CausalFlowMatchScheduler
from teletron.models.causwan.modules.tokenizers import HuggingfaceTokenizer
from teletron.models.causwan.modules.model import WanModel, RegisterTokens, GanAttentionBlock
from teletron.models.causwan.modules.vae import _video_vae
from teletron.models.causwan.modules.t5 import umt5_xxl
from teletron.models.causwan.modules.causal_model import CausalWanModel
from teletron.models.causwan.modules.causal_fps_model import CausalFPSWanModel
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load("/nvfile-heatstorage/model_zoo/modelscope/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth",
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name="/nvfile-heatstorage/model_zoo/modelscope/Wan2.1-T2V-1.3B/google/umt5-xxl/", seq_len=512, clean='whitespace')#wan_models/Wan2.1-T2V-1.3B/
    ...
    def forward(self, text_prompts: List[str]) -> dict:
        ...
        # ids = ids.to(next(self.text_encoder.parameters()).device)
        # mask = mask.to(next(self.text_encoder.parameters()).device)
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path="/nvfile-heatstorage/model_zoo/modelscope/Wan2.1-T2V-1.3B/Wan2.1_VAE.pth",
            z_dim=16,
        ).eval().requires_grad_(False)
    ...
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                ...
                "/nvfile-heatstorage/model_zoo/modelscope/Wan2.1-T2V-1.3B/", local_attn_size=local_attn_size, sink_size=sink_size)
        else:
            self.model = WanModel.from_pretrained("/nvfile-heatstorage/model_zoo/modelscope/Wan2.1-T2V-1.3B/")
        self.model.eval()

        # For non-causal diffusion, all frames share the same timestep
        self.uniform_timestep = not is_causal

        self.scheduler = CausalFlowMatchScheduler(
...
class WanFPSWrapper(torch.nn.Module):
    def __init__(
            self,
            model_name="Wan2.1-T2V-14B",
            timestep_shift=8.0,
        ...
        if is_causal:
            self.model = CausalFPSWanModel.from_pretrained(
                f"/nvfile-heatstorage/model_zoo/modelscope/Wan2.1-T2V-1.3B/", local_attn_size=local_attn_size, sink_size=sink_size)
        else:
            self.model = WanModel.from_pretrained(f"/nvfile-heatstorage/model_zoo/modelscope/Wan2.1-T2V-1.3B/")
        self.model.eval()
